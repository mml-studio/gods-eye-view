...
| Barrages ▰ | OSM via Overpass for France + OpenInfraMap snapshot elsewhere (bundled, **7 432 features**) — `height` is MEASURED at 171 of 7 432 (2.30 %) and therefore REFUSED, with the refusal locked by an assertion: at that coverage it would be a map of 171 objects and 7 261 defaults. The size channel carries `spanM` instead, the longest dimension measured on the geometry at build time, present on 5 328 (71.69 %), 25 m → 6 399 m (median 100, p95 539), in CONSTANT SCREEN PIXELS — four frozen span classes (100/300/1 000 m) at 18/13/9/6 px plus a hollow 8 px ring for the 2 104 unmeasured (28.3 %). The diameters are NOT proportional to span and the legend publishes classes with their bounds in metres rather than a false continuous scale. The old editorial importance tier no longer drives dot size or stem width; it survives on label priority and card range | `src/data/localLayers.js`, `src/data/damsPack.js`, `src/data/localGeojson.js` | — | static |
