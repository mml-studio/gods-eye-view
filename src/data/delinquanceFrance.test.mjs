...
test('a commune card under the total prints the floor, the gap, and the authorship', () => {
  ...
  assert.match(copy, /Total CALCULÉ par Surplomb, non publié par le SSMSI/);
  assert.match(quoted, /Total CALCULÉ par Surplomb, pas publié par le SSMSI/);
...
test('a département card under the total says the total is exact THERE and not below', () => {
  ...
  for (const text of [copy, quoted]) assert.match(text, /Total CALCULÉ par Surplomb/);
